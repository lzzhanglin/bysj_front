<template>
    <div>
                ...
                <el-form ref="videoForm" :model="videoForm"  :rules="rules" label-width="80px" >
                    ...
                    <el-form-item label="视频上传" v-if="role==='ROLE_TEACHER'">
                        <el-upload
                            ...
                            <div slot="tip" class="el-upload__tip" style="color:#3a8ee6;">选择课程和章节之后方可进行上传，播放视频，只能上传 MP4 格式视频</div>
                        </el-upload>
        ...
        <div class="container" style="padding-top:0px;" v-if="isChoose" >
              <div slot="tip" v-if="isEmpty" style="color:red;font-size:18px;text-align:center;margin-top:30px;">该章节暂无教学视频</div>
       

                <div v-for="video in videoList" :key='video.videoId'>
                  
                   <div class="plugins-tips"  style="margin-top:40px;margin-left:80px;width:600px;">
                        <p align="center">{{video.videoName}}</p>
                    </div>
   
                        <el-button style="margin-top:10px; margin-bottom:10px;" size="small" type="danger" @click="handleDelete(video.videoId)" v-if="role==='ROLE_TEACHER'" >删 除</el-button>

                   
                   <div>
                            <video :src="video.externalLink" controls style="margin-top:10px; width:800px;">
                                                您的浏览器不支持Video标签。
                                            </video>
                   </div>
