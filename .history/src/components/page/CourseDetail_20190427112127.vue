       ...
       return params
          ...
          return params
           ...
           //监听路由的变化，查看课程的页面可以同时打开多个课程的详情页，但是每个页面共用一个组件，需要使用各自的courseId加载章节列表
            watch: {
                '$route' (to, from) {
                    var toPath = to.fullPath.substr(0,14).tostring;
                    var fromPath = from.fullPath.substr(0,14).tostring;
                    console.log(fromPath+" ----> "+toPath);
                    console.log(toPath == '/course/datail');
                    // debugger;
                        //主url相同 id不同的时候才进行操作
                        if(toPath == '/course/datail'){
                            console.log("进行获取课程id操作和获取章节列表操作");
                            this.getParams();
                            this.getData();
                        }
                ...
                console.log("课程id为："+this.$route.query.id)
                }
            },
