<template>
    <div>
        ...
        <div class="container" v-if="isChoose" >
              <div slot="tip" v-if="isEmpty" style="color:red;font-size:18px;text-align:center;">该章节暂无课件</div>
                ...
                <div v-for="courseWare in courseWareList" :key='courseWare.courseWareId'>
                    <div  v-bind="currentCourse">-------------></div>
                    <div  v-bind="currentChapter"></div>
                   <div class="plugins-tips"  style="width:100%;margin-top:0px;">
                        <p align="center">{{courseWare.courseWareName}}</p>
                    </div>
   
                        <el-button style="margin-top:10px; margin-bottom:10px;" size="small" type="danger" @click="handleDelete(courseWare.courseWareId)" v-if="role==='ROLE_TEACHER'" >删 除</el-button>

                       <iframe :src="courseWare.previewLink"  width='100%' height='1000px' frameborder='0' margin-top='10px'>
             ...
             chooseChapter(){
                 this.disableUpload = false;
                  ...
                  for(let i = 0;i < courses.length;i++){
                      if (courses[i].courseId == this.courseWareForm.courseId){
                          this.currentCourse =courses[i].courseName;
                      }
                  }
                   for(let i = 0;i < chapters.length;i++){
                      if (chapters[i].chapterId == this.courseWareForm.chapterId){
                          this.currentChapter =chapters[i].chapterName;
                      }
                  }
            },
