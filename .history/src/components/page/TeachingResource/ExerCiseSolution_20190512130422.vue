    ...
    <div>
        ...
        <div class="container" >
            <div class="form-box">
                ...
                <el-form ref="solutionForm" :model="solutionForm"  :rules="rules" label-width="80px" >
                    ...
                    <el-form-item label="课件上传" v-if="role==='ROLE_TEACHER'">
                        <el-upload
                            class="upload-demo"
                            ref="upload"
                            :limit="1"
                            ...
                            <div slot="tip" class="el-upload__tip" style="color:#3a8ee6;">选择课程和对应的习题之后方可进行上传，预览参考答案，只能上传 word 文档</div>
                        </el-upload>
             ...
             <div class="crumbs" v-if="isChoose">
                    <el-breadcrumb separator="/">
                        <el-breadcrumb-item><i class="el-icon-lx-calendar"></i> 教学资源</el-breadcrumb-item>
                        <el-breadcrumb-item> 参考答案预览</el-breadcrumb-item>
        ...
        <div class="container" style="padding-top:0px;" v-if="isChoose" >
              <div slot="tip" v-if="isEmpty" style="color:red;font-size:18px;text-align:center;margin-top:30px;">该习题暂无参考答案</div>
       
   
                <div v-for="solution in solutionList" :key='solution.solutionId'>
                  
                   <div class="plugins-tips"  style="margin-top:40px;margin-right:4px;">
                        <p align="center">
                            {{solution.solutionName}}</p>
                    </div>
                        <el-button style="margin-top:10px; margin-bottom:10px;" size="small" type="primary" @click="handleDownload(solution.externalLink)" >下 载</el-button>
   
                        <el-button style="margin-top:0px; margin-bottom:10px;" size="small" type="danger" @click="handleDelete(solution.solutionId)" v-if="role==='ROLE_TEACHER'" >删 除</el-button>

                       <iframe :src="solution.previewLink" margin-nottom='20px' width='100%' height='1000px' frameborder='0' margin-top='10px'>
        ...
        <el-dialog title="提示" :visible.sync="delVisible" width="300px" center>
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="deleteSolution">确 定</el-button>
            </span>
        </el-dialog>
         ...
         onSuccess(response,file) {
            ...
            //在线预览的链接 加上微软的链接 word ppt 都需要微软的服务 pdf jpg png不需要 可以直接预览
             var previewLink = "https://view.officeapps.live.com/op/embed.aspx?src="+this.externalLink;
                ...
                this.postRequest('/api/solution/writeFileInfo', {
                            solutionName: response.key,
                            externalLink: this.externalLink,
                            previewLink: previewLink,
                            creatorJobNo: window.localStorage["jobNo"],
                            courseId: this.solutionForm.courseId,
                            exerciseId: this.solutionForm.exerciseId,
                  }).then(resp=> {
            ...
            chooseCourse(){
             ...
             chooseExercise(){
                 this.disableUpload = false;
                  ...
                  for(let i = 0;i < this.courses.length;i++){
                      if (this.courses[i].courseId == this.solutionForm.courseId){
                          this.currentCourse =this.courses[i].courseName;
                      }
                  }
                   for(let i = 0;i < this.exercises.length;i++){
                      if (this.exercises[i].courseWareId == this.solutionForm.exerciseId){
                          this.currentExercise =this.exercises[i].courseWareName;
                      }
                  }
            },
             ...
             getSolutionList() {
                var dataFromDb = [];
              let _this = this
                this.$axios.get('/api/solution/getSolutionList', {
                    params:{
                        exerciseId:this.solutionForm.exerciseId
                    }
                }).then((res) => {
           ...
           getExerciseList() {
             ...
             handleDelete(solutionId) {
                ...
                this.deleteSolutionId = solutionId;
            },
     
            deleteSolution(solutionId){
                  this.$axios.delete('/api/solution/delete', {
                    params: {
                        solutionId: this.deleteSolutionId
                    }
                }).then((res) => {
