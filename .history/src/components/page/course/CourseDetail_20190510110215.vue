           ...
           saveEdit(index,row){
               ...
               var chooseChapterId;
                 for(let i = 0;i<this.options.length;i++){
                     if(this.options[i].courseId == this.chapterForm.courseId){
                         chooseCourseName = this.options[i].courseName;
                         chooseChapterId = this.options[i].chapterId
                     }
                 }
            ...
            //****************** */
             ...
             handleView(index, row){
                const item = this.tableData[index];
                 this.$router.push({
                   path: '/course/chapter', query:{id: item.chapterId, name: item.chapterName}
        });
             },
