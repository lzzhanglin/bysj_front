        ...
        <div class="container">
            <div class="form-box">
                <el-form ref="courseForm" :model="courseForm"  :rules="rules" label-width="80px">
                    ...
                    <el-form-item label="开课日期" prop="dateBegin" label-width="80px">
                        <el-col :span="11">
                            <el-date-picker type="date" placeholder="选择开课日期"  :picker-options="dateBegin"
    ...
    export default {
        ...
        data: function(){
            ...
            return {
                 ...
                 dateBegin: {
                disabledDate: (dateBegin) => {
                    if (this.courseForm.dateEnd) {
                        //如果选择了结课日期，只能选则今天之后的日期 且 结课日期之前的日期
                        return dateBegin.getTime() < Date.now() || dateBegin.getTime() >  new Date(this.courseForm.dateEnd).getTime() -  8.64e7;
 
                }else{//还没有选择结束时间的时候，让他只能选择今天之后的时间 包括今天
                        return dateBegin.getTime() < Date.now() - 8.64e7;
                    } 
            }
                 },
            dateEnd: {
                disabledDate: (dateEnd) => {
                    //结课日期必须大于开课日期
                    return dateEnd.getTime() < new Date(this.courseForm.dateBegin).getTime();
                }
            },
